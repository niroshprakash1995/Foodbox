const productsEl = document.getElementById('searchproducts');
	...
	$.ajax({
		...
		success(res) {
			...
			displayProductsDOM(JSON.parse(lsData));
				...
				alert("No foods found for this name. Please again.")
...
function displayProductsDOM(searchproducts) {
	productsEl.innerHTML = searchproducts.map(product => `
        <div class="product">
