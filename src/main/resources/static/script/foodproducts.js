const productsEl = document.getElementById('foodproducts');
...
// Loads product data 
const loadProducts = () => {
	var cuisineName = localStorage.getItem("cuisineName");
	var inputData;
	$.ajax({
		url: '/fetchcuisinefoods',
		method: "POST",
		data: {
		...
		success(res) {
			...
			displayProductsDOM(JSON.parse(lsData));
				...
				alert("No foods found for this cuisine. Please try a different cuisine.")
...
function displayProductsDOM(foodproducts) {
	productsEl.innerHTML = foodproducts.map(product => `
        <div class="product">
...
function isItemInCart(currId) {
	for (const product of cartArr) {
		if (parseInt(currId) === product._id) {
			product['qty'] += 1;
			return true;
		}
	}
...
function addToCart(e) {
	...
	if (!isItemInCart(currId)) {
		for (const product of productsArr) {
			if (product._id === parseInt(currId)) {
